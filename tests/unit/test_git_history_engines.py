...
def git_repo(tmp_path: Path) -> Path:
    ...
    file_a.write_text("one\ntwo\n", encoding="utf-8")
    subprocess.run(["git", "add", "."], cwd=repo, check=True)
    subprocess.run(["git", "commit", "-m", "add second line"], cwd=repo, check=True)
...
def test_parse_git_log_line_valid():
...
def test_get_commit_stats(git_repo: Path):
    repo_info = find_git_repository_root(git_repo)
    commit = get_commit_log(repo_info)[0]

    stats = get_commit_stats(repo_info, commit.commit_hash)
...
def test_attach_commit_stats(git_repo: Path):
    ...
    assert all(c.stats is not None for c in enriched)
...
def test_compute_author_contributions(git_repo: Path):
    repo_info = find_git_repository_root(git_repo)
    commits = attach_commit_stats(repo_info, get_commit_log(repo_info))

    contrib = compute_author_contributions(commits)
    assert len(contrib) >= 1


def test_compute_bus_factor_single_author():
    contributions = {
...
def test_detect_inactivity_recent_commits(git_repo: Path):
    repo_info = find_git_repository_root(git_repo)
    commits = get_commit_log(repo_info)
...
def test_is_meaningful_commit_message():
    assert is_meaningful_commit_message("add feature X") is True
    assert is_meaningful_commit_message("fix") is False
...
def test_analyze_commit_messages(git_repo: Path):
    ...
    assert 0.0 <= quality["meaningful_ratio"] <= 1.0
...
def test_extract_file_level_changes(git_repo: Path):
    ...
    impacts = extract_file_level_changes(repo_info, commit.commit_hash)
...
def test_aggregate_file_impact_across_commits(git_repo: Path):
    ...
    impacts = aggregate_file_impact_across_commits(repo_info, commits)
    assert isinstance(impacts, dict)


def test_compute_churn_metrics_empty():
...
def test_detect_merge_commits_no_merges(git_repo: Path):
    repo_info = find_git_repository_root(git_repo)
    commits = get_commit_log(repo_info)
